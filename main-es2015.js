...
let DashboardComponent = class DashboardComponent {
    constructor(firestore, _router) {
        this.firestore = firestore;
        this._router = _router;
    }
    ngOnInit() {
        this.firestore.collection("/bookings").snapshotChanges().subscribe((res) => {
            this.bookings = res.map(item => {
    ...
    delete(id) {
        ...
        this.firestore.doc("/bookings/" + id).delete().then((res) => {
            console.log(res);
        }).catch((err) => {
            console.log(err);
        });
...
const routes = [{ path: '', component: _layout_layout_component__WEBPACK_IMPORTED_MODULE_13__["LayoutComponent"], children: [
...
let FormsComponent = class FormsComponent {
    ...
    submit(frm) {
        this.http.post(this.emailurl, JSON.stringify(frm.value)).subscribe((result) => {
            alert("email Sent");
        });
        this.firestore.collection("/bookings").add(frm.value).then((res) => {
            console.log(res);
        });
    }
    resetForm(frm) {
